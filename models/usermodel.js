...
module.exports = {
  ...
  activate: (confirmation, callback) => {
    db.connection.query('UPDATE users SET active = ? WHERE confirmation = ?', [1, confirmation], function(error, result) {
      if (error) {
        return callback(error, null);
      }
  ...
  updateConfirmation: (confirmation, callback) => {
    db.connection.query('UPDATE users SET confirmation = ? WHERE confirmation =?', ['0', confirmation], function(error, result) {
      if (error) {
        return callback(error, null);
      }
